// Copyright 2010-2012 Spraylight GmbH

#ifndef __MURL_GRAPH_I_LISTENER_STATE_H__
#define __MURL_GRAPH_I_LISTENER_STATE_H__

#include "murl_graph_i_listener.h"
#include "murl_graph_i_state_slot.h"

namespace Murl
{
    namespace Graph
    {
        /**
         * @ingroup MurlGraphNodeInterfaces
         * @interface IListenerState
         * @brief The IListenerState graph node interface.
         * Listener state nodes are used to activate and/or quickly switch between listeners
         * during scene graph traversal. As soon as a specific listener is activated via a
         * listener state node, it stays active during traversal until a different listener
         * state node is encountered.
         *
         * See @ref sceneGraphStatesSlotsUnits for an overview of state handling during scene
         * graph traversal.\n
         * See Graph::IListener for defining a specific listener for rendering.\n
         * See Graph::IListenerTransform for setting a listener's position and orientation.
         */
        class IListenerState : public IStateSlot
        {
        public:
            /**
             * @brief Get the mutable Graph::INode interface.
             * This method returns a mutable pointer to the node's Graph::INode interface, to
             * be able to query or modify common node properties such as active state,
             * visibility or ID.
             * @return The mutable Graph::INode interface, or null if not available
             */
            virtual INode* GetNodeInterface() = 0;
            /**
             * @brief Get the constant Graph::INode interface.
             * This method returns a constant pointer to the node's Graph::INode interface, to
             * be able to query common node properties such as active state, visibility
             * or ID.
             * @return The constant Graph::INode interface, or null if not available
             */
            virtual const INode* GetNodeInterface() const = 0;
            
            /**
             * @brief Get the mutable Graph::IListenerNodeTarget container.
             * This method returns a mutable pointer to the node's Graph::IListenerNodeTarget
             * container, which allows to set or query the listener this state node refers to.
             * @return The mutable Graph::IListenerNodeTarget container, or null if not available.
             */
            virtual IListenerNodeTarget* GetListenerNodeTarget() = 0;
            /**
             * @brief Get the constant Graph::IListenerNodeTarget container.
             * This method returns a constant pointer to the node's Graph::IListenerNodeTarget
             * container, which allows to query the listener this state node refers to.
             * @return The mutable Graph::IListenerNodeTarget container, or null if not available.
             */
            virtual const IListenerNodeTarget* GetListenerNodeTarget() const = 0;
            
        protected:
            virtual ~IListenerState() {}
        };
        
        /**
         * @ingroup MurlGraphTargetInterfaces
         * @brief A container for referencing one or more Graph::IListenerState nodes.
         */
        typedef IGenericNodeTarget<IListenerState> IListenerStateNodeTarget;
    }
}

#endif // __MURL_GRAPH_I_LISTENER_STATE_H__
